...
const cacheThisMemoryDynamic = async () => {
  'use cache';
  cacheLife('days');
  cacheTag('cacheThisMemoryDynamic');
...
const DataFetcher = async () => {
  await connection();
  const timestamp = await cacheThisMemoryDynamic();
  return <p>Data fetched at: {new Date(timestamp).toLocaleTimeString()}</p>;
}

export default async function MemoryDynamic() {
  return (
    <div className="flex flex-col flex-1 items-center justify-center bg-zinc-50 font-sans dark:bg-black">
      <main className="flex flex-1 w-full max-w-3xl flex-col items-center py-32 px-16 bg-white dark:bg-black sm:items-start">
        ...
        <button type='button' className="bg-foreground text-background px-4 py-2 rounded m-6 cursor-pointer" onClick={async () => {
          'use server';
          revalidateTag('cacheThisMemoryDynamic', { expire: 60 })
        }}>
          Revalidate
        </button>
      </main>
