...
const cacheThisAutoRemoteStatic = async () => {
  'use cache: remote';
  cacheLife({ stale: 60, expire: 3600, revalidate: 10 });
  cacheTag('cacheThisAutoRemoteStatic');
  await new Promise((resolve) => setTimeout(resolve, 2000));
  return Date.now();
};

const DataFetcher = async () => {
  const timestamp = await cacheThisAutoRemoteStatic();
  return <p>Data fetched at: {new Date(timestamp).toLocaleTimeString()}</p>;
}

export default async function AutoRemoteStatic() {
  return (
